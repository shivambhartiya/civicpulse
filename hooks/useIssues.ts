...
export function useIssues() { const [issues, setIssues] = useState<Issue[]>(mockIssues); const [loading, setLoading] = useState(false); useEffect(() => { if (!hasFirebaseConfig || !db) return; setLoading(true); return onSnapshot(query(collection(db, 'issues'), orderBy('reportedAt', 'desc'), limit(50)), (snap) => { setIssues(snap.docs.map((d) => ({ id: d.id, ...d.data() } as Issue))); setLoading(false); }, () => setLoading(false)); }, []); return { issues, loading }; }
